...
object DefTest {
  ...
  def main(args: Array[String]) : Unit = {
    ...
    // def 类型的变量, 在声明赋值时, 右面的变化是不会马上计算结果的
    // 在def变量每次被调用的时候, 等号右边的表达式是都会被重新计算一次的
    def d = sumInt(3,5)
    ...
    // lazy 定义变量
    // lazy 变量, 在声明赋值时, 等号右边的表达式不会马上计算结果
    // lazy 在对象第一次调用的时候, 等号右边表达式会计算一次,并赋值给对象
    // 后续的对lazy对象的再次调用, 右边的表达式是将不会再被重新计算的
